      ...
      },
      {
        ...
        if (!rawPassed) {
          return { code: -1, message: 'AI 返回内容未通过安全检测，请重试', data: null }
	        ...
	        console.error('[ai-shopping] raw msgSecCheck 失败:', e.errCode)
	    ...
	    const textsToCheck = [
	      result.summary || '',
	      ...(result.items || []).map(i => i.name || '').filter(Boolean).slice(0, 20)
	    ].filter(Boolean)
        ...
        if (!passed) {
          return { code: -1, message: 'AI 生成的采购清单未通过安全检测，请重试', data: null }
        }
	      } catch (e) {
	        console.error('[ai-shopping] msgSecCheck 失败:', e.errCode, e.message || e.errMsg)
	        return { code: -1, message: '内容安全检查暂时不可用，请稍后重试', data: null }
	      }
	    }
